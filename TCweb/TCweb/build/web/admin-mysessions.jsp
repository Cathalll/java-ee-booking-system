...
<body>
    ...
    <h1>Learning Dashboard - Admin</h1>
            ...
            <c:forEach items="${autumnSessions}" var="autumnSess" varStatus="loop">
                ...
                <tr>
                    ...
                    <td>${autumnSess.classroom.name}</td>
                     <td>${autumnSess.teacher.name} ${autumnSess.teacher.surname}<br>
                        <a href="mailto:${autumnSess.teacher.email}">${autumnSess.teacher.email}</a></td>
                    <td>
                
                        <strong>${fn:length(autumnSess.students)} enrolled </strong>/ ${autumnSess.capacity} total<br>
                        
                        <a href="${pageContext.request.contextPath }/studentlistadmin?id=${autumnSess.id}">List students</a>
            ...
            <c:forEach items="${springSessions}" var="springSess" varStatus="loop">
                ...
                <tr>
                    ...
                    <td>${springSess.classroom.name}</td>
                      <td>${springSess.teacher.name} ${springSess.teacher.surname}<br>
                        <a href="mailto:${springSess.teacher.email}">${springSess.teacher.email}</a></td>
                    <td>
                        <strong>${fn:length(springSess.students)} enrolled </strong>/ ${springSess.capacity} total<br>
                        
                         <a href="${pageContext.request.contextPath }/studentlistadmin?id=${springSess.id}">List students</a>
